...
async def test_users_views_get_user_account_success() -> None:
    ...
    user: UserModel = await UsersViews(uow=users_unit_of_work).get_user_account(user_id=FakeUserConfig.ID)
...
async def test_users_views_get_user_account_fails_user_does_not_exist() -> None:
    ...
    with pytest.raises(UserNotFoundError):
        await UsersViews(uow=users_unit_of_work).get_user_account(user_id=FakeUserConfig.ID)
